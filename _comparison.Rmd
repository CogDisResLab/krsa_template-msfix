...
if (!length(figure_output)==0) {
  graphics.off()
  pdf(filename=paste(figure_folder, "{{case}}_vs_{{ctrl}}_heatmap", sep="/"))
  grid::grid.newpage()
...
if (!length(figure_output)==0) {
  ...
  "Barcode",
  groups = c("{{case}}", "{{ctrl}}"))
) |> ggsave(filename=paste(figure_folder, "{{case}}_vs_{{ctrl}}_violinplot.svg", sep="/"))
  } else {
  krsa_violin_plot(data_modeled[["scaled"]],
  significant_peptides[["meanLFC"]][["0.2"]],
  "Barcode",
...
if (!length(figure_output)==0) {
 ...
 |> ggsave(filename=paste(figure_folder, "{{case}}_vs_{{ctrl}}_krsawaterfall.svg", sep="/"))
  } else {
  krsa_waterfall(differential_phosphorylated_peptides, lfc_thr = 0.2, byChip = FALSE)
  }
...
if (!length(figure_output)==0) {
  krsa_zscores_plot(AvgZTable2)
 |> ggsave(filename=paste(figure_folder, "{{case}}_vs_{{ctrl}}_krsazscoresplot.svg", sep="/"))
  } else {
  krsa_zscores_plot(AvgZTable2)
  }
...
if (!length(figure_output)==0) {
 ...
 |> ggsave(filename=paste(figure_folder, "{{case}}_vs_{{ctrl}}_krsarevplot.svg", sep="/"))
  } else {
  krsa_reverse_krsa_plot(chipCov, differential_phosphorylated_peptides, kinases_hits, 0.2, byChip = FALSE)
  }
...
if (!length(figure_output)==0) {
  krsa_coverage_plot(chipCov, AvgZTable2, chipType)
 |> ggsave(filename=paste(figure_folder, "{{case}}_vs_{{ctrl}}_krsacovplot.svg", sep="/"))
  } else {
  krsa_coverage_plot(chipCov, AvgZTable2, chipType)
  }
...
nball <- krsa_ball_model(kinases_hits, AvgZTable2, 10L, 2.5, 4.8)
