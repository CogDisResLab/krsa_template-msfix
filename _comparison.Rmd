...
data_modeled[["scaled"]] |>
  ...
  write_csv(
    file.path(
      "results",
      "{{run_prefix}}-filtered_peptides_{{case}}_{{ctrl}}-{{chip}}.csv"
    )
  )
...
write_csv(
  ...
  file.path(
    "results",
    "{{run_prefix}}-dpp_{{case}}_{{ctrl}}-{{chip}}.csv"
  )
)
...
krsa_heatmap(data_modeled[["normalized"]], significant_peptides[["meanLFC"]][["0.2"]],
...
krsa_violin_plot(data_modeled[["scaled"]], significant_peptides[["meanLFC"]][["0.2"]],
  "Barcode",
  groups = c("{{case}}", "{{ctrl}}")
)
```
...
krsa_waterfall(differential_phosphorylated_peptides,
  lfc_thr = 0.2, byChip = FALSE
)
```
