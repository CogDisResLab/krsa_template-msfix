...
if (params[["paired"]] & !(params[["pairing_variable"]] %in% colnames(meta_data))) {
  stop("Parameter pairing_variable not found in metadata_file column names! Is the pairing_variable present in colnames(metadata_file) and spelled correctly?")
}

raw_data <- meta_data |> group_by(run_id) |>
  summarize(data=list(map2_df(krsa_read(signal_file=median_sigmb_file,saturation_file=signal_saturation_file)))) |>
  unnest(data)

stop(paste("raw_data dims:", dim(raw_data)))
```
