...
sample_name_to_run_id <- qc_data |> select(SampleName, run_id) |> distinct()
...
data_pw <- krsa_extractEndPoint(qc_data, chipType) |> left_join(sample_name_to_run_id, by=c("SampleName")) |> select(SampleName, Peptide, ExposureTime, Signal, Barcode, Group, run_id)
...
wide_data_temp <- data_pw |>
  ...
  select(Peptide, sample_exposure_id, SampleName, ExposureTime, Signal, Group, run_id, Barcode) # Only relevant columns for pivot

design_df <- wide_data_temp |> select(sample_exposure_id, SampleName, ExposureTime, Group, run_id, Barcode) |> distinct()
...
sva_data_matrix <- data_matrix_wide |> as.matrix()
sva_data_matrix <- sva_data_matrix[-na_indices,]
colnames(sva_data_matrix) <- column_names
rownames(sva_data_matrix) <- peptide[-na_indices]
mod <- model.matrix(~ExposureTime + Group, data = design_df)

mod0 <- model.matrix(~1, data = design_df)

svobj <- sva(dat = sva_data_matrix, mod = mod, mod0 = mod0, n.sv = NULL)

mod_with_sva <- cbind(mod, svobj$sv)

adjusted_data_combat_sva <- ComBat(dat = sva_data_matrix,
                                   batch = as.factor(design_df$run_id),
                                   mod = mod_with_sva, # Use the design matrix including SVs
                                   par.prior = TRUE,
                                   prior.plot = FALSE)
